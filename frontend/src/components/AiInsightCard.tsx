...
export default function AiInsightCard({
  topCategoryName = "No expenses yet",
  topCategoryAmount = 0,
}: AiInsightCardProps) {
    ...
    <div className="rounded-2xl border border-cyan-900 bg-cyan-950/40 p-6">
      ...
      {topCategoryAmount > 0 ? (
        <p className="mt-4 text-sm leading-6 text-slate-300">
          ...
          {topCategoryAmount.toFixed(2)}. Reducing this category by 10% could
          save about ${potentialSavings.toFixed(2)} this month.
        </p>
      ) : (
        <p className="mt-4 text-sm leading-6 text-slate-300">
          Add expense transactions to generate a spending insight.
        </p>
      )}
